...
public class SimpleDroneController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (Mobile == false)
        {
            ...
            if (Input.GetKey(KeyCode.LeftArrow) || Input.GetAxis("XRI_Left_Primary2DAxis_Vertical") <= -0.1f) { Drone.AddRelativeForce(FlyLeftRight / -1, 0, 0); Drone.AddRelativeTorque(0, 0, 10); }//rotate drone left

            if (Input.GetKey(KeyCode.RightArrow) || Input.GetAxis("XRI_Left_Primary2DAxis_Vertical") >= 0.1f) { Drone.AddRelativeForce(FlyLeftRight, 0, 0); Drone.AddRelativeTorque(0, 0, -10); }//rotate drone right

            if (Input.GetKey(KeyCode.S)) { Drone.AddRelativeForce(0, 0, ForwardBackward / -1); Drone.AddRelativeTorque(-10, 0, 0); }// drone fly backward

            if (Input.GetKey(KeyCode.UpArrow) || Input.GetAxis("XRI_Left_Primary2DAxis_Horizontal") >= 0.1f) { Drone.AddRelativeForce(0, UpDown, 0); }//drone fly up

            if (Input.GetKey(KeyCode.DownArrow) || Input.GetAxis("XRI_Left_Primary2DAxis_Horizontal") <= -0.1f) { Drone.AddRelativeForce(0, UpDown / -1, 0); }//drone fly down
        }
        if (Mobile == true)
        {
